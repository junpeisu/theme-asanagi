 ...
 */
function asanagi_id_safe($string) {
 ...
 */
function asanagi_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
 ...
 */
function asanagi_comment_wrapper($content, $node) {
 ...
 */
function asanagi_node_submitted($node) {
...
function asanagi_comment_submitted($comment) {
 ...
 */
function asanagi_preprocess_block(&$vars, $hook) {
  $block = $vars['block'];
  ...
  if (user_access('administer blocks')) {
    include_once './' . drupal_get_path('theme', 'asanagi') . '/template.block-editing.inc';
    asanagi_preprocess_block_editing($vars, $hook);
